...
def count_files(path: str) -> Tuple[bool, int]:
    """
        Recursively counts all files in the specified directory and its subdirectories.
...
def count_bytes(path: str) -> Tuple[bool, int]:
    """
       Recursively calculates the total size of all files in the specified directory.
...
def analyze_windows_file_types(path: str) \
    ...
    """
       Analyzes file types by extension in the specified directory and its subdirectories.
...
def get_windows_file_attributes_stats(path: str) -> Dict[str, int]:
    """
       Collects Windows file attribute statistics for the specified directory.
...
def show_windows_directory_stats(path: str) -> bool:
    """
      Displays comprehensive directory statistics including file counts, sizes,
      file types, and Windows file attributes.
    ...
    if ok_types:
        ...
        for ext, info in sorted(types.items(), key=lambda x: -x[1]['count']):
            print(f"  {ext or '[no extension]'}: {info['count']} files, {utils.format_size(info['bytes'])}")
